...
**核心原则**: 对于 >= 64KB 的消息，AXON 库相比原生 UCX 的 overhead 不超过 5%；对于 < 4KB 的小消息，overhead 不超过 2us (绝对值)。
...
from axon import Endpoint, Config
...
#!/bin/bash
# 使用 MPI 启动跨节点测试
# mpirun -np 2 --hostfile hostfile ./axon_integration_test

# 或使用 SSH + 自定义脚本
NODE_A="10.0.0.1"
NODE_B="10.0.0.2"

ssh $NODE_A "./axon_test_server --port 12345 --transport rdma" &
SERVER_PID=$!
sleep 2
ssh $NODE_B "./axon_test_client --server $NODE_A:12345 --transport rdma --size 1M"
CLIENT_EXIT=$?
