...
3. UCP 的 tag-matching 和 Active Message API 可直接映射到 AXON KV 传输语义
...
#define AXON_REGISTER_PLUGIN(PluginClass) \
...
**加载方式：**
1. **静态链接**：编译时通过 CMake `option(AXON_ENABLE_NCCL ...)` 控制
2. **动态加载**：运行时通过 `PluginRegistry::load_plugin("libaxon_nccl.so")` 或环境变量 `AXON_PLUGINS=nccl,hccl`
3. **自动发现**：扫描 `${AXON_PLUGIN_DIR}/*.so`，调用约定的 `axon_plugin_create()` 入口函数
