...
Demonstrates the Python API for the P2P transport library.
"""
...
async def example_basic():
    ...
    await ep.tag_send(data, tag=p2p.make_tag(0, 42))
    ...
    print(f"Received {nbytes} bytes, tag={p2p.tag_user(matched_tag)}")
...
async def example_registered_memory():
    ...
    region = p2p.MemoryRegion.register(ctx, big_array)

    # Hot loop: send from pre-registered memory (true zero-copy path)
    for i in range(100):
        await ep.tag_send(region, tag=p2p.make_tag(0, i))
...
async def example_memory_pool():
    ...
    ep = await worker.connect("10.0.0.1:13337")
    pool = p2p.MemoryPool.create(ctx, pool_bytes=64 * 1024 * 1024)
    ...
    for i in range(1000):
        ...
        task = asyncio.create_task(ep.tag_send(buf, tag=p2p.make_tag(0, i)))
...
async def example_rdma():
    ...
    local = p2p.MemoryRegion.allocate(ctx, 4096)
...
async def example_server():
    ...
    async def handle_client(ep: p2p.Endpoint):
        """Handle one client connection."""
        buf = np.empty(1024 * 1024, dtype=np.uint8)
        try:
            while ep.is_connected:
                nbytes, tag = await ep.tag_recv(buf, tag=p2p.TAG_ANY)
                # Echo back
                await ep.tag_send(
                    memoryview(buf)[:nbytes], tag=tag
                )
        except p2p.P2PError as e:
            if e.code != p2p.ErrorCode.ENDPOINT_CLOSED:
                raise

    clients: list[asyncio.Task] = []

    def on_accept(ep: p2p.Endpoint):
...
async def example_registration_cache():
    ...
    ep = await worker.connect("10.0.0.1:13337")
    cache = p2p.RegistrationCache.create(ctx, max_entries=1024)
...
async def example_concurrent():
    ...
    await asyncio.gather(
        *(ep.tag_send(arr, tag=p2p.make_tag(0, i)) for i, arr in enumerate(arrays))
    )

    # Fire 10 receives concurrently
    recv_bufs = [np.empty(1024, dtype=np.float32) for _ in range(10)]
    results = await asyncio.gather(
        *(ep.tag_recv(buf, tag=p2p.make_tag(0, i)) for i, buf in enumerate(recv_bufs))
    )

    for nbytes, matched_tag in results:
        print(f"  tag={p2p.tag_user(matched_tag)}, bytes={nbytes}")
