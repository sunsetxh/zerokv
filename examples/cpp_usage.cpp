...
/// @brief Demonstrates how to use the P2P library for point-to-point communication.
///
/// Build:  g++ -std=c++20 -I../include cpp_usage.cpp -lp2p -lpthread -o demo
...
void example_tag_messaging() {
    ...
    auto cfg = p2p::Config::builder()
                   ...
                   .from_env()  // override with P2P_* env vars
                   .build();

    auto ctx = p2p::Context::create(cfg);

    // --- Sender side ---
    auto sender_worker = p2p::Worker::create(ctx, /*index=*/0);
    ...
    p2p::Tag tag = p2p::make_tag(/*context_id=*/0, /*user_tag=*/42);
...
void example_rdma() {
    ...
    // --- Local memory ---
    auto local_region = p2p::MemoryRegion::allocate(ctx, 4096);
    ...
    p2p::RemoteKey remote_rkey;      // placeholder
...
void example_memory_pool_and_callbacks() {
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        p2p::Tag tag = p2p::make_tag(0, static_cast<uint32_t>(i));

        // Use callback-based completion (avoids polling overhead)
        auto future = ep->tag_send(buf.region, tag);
        future.on_complete([&pool, buf_copy = buf](p2p::Status st, auto) mutable {
            if (!st.ok()) {
...
void example_batch() {
    ...
    std::vector<p2p::Future<void>> futures;
    futures.reserve(100);

    for (int i = 0; i < 100; ++i) {
        float val = static_cast<float>(i);
        futures.push_back(ep->tag_send(&val, sizeof(val), p2p::make_tag(0, i)));
    }

    // Wait for all sends to complete (with timeout)
    auto status = p2p::wait_all(futures, std::chrono::seconds{5});
