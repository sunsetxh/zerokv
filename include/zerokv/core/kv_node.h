...
public:
    ...
    [[nodiscard]] zerokv::transport::MemoryRegion::Ptr allocate_region(size_t size) const;
    ...
    zerokv::transport::Future<void> publish(const std::string& key,
                                 const void* data,
                                 size_t size);
    zerokv::transport::Future<void> publish_region(const std::string& key,
                                        const zerokv::transport::MemoryRegion::Ptr& region,
                                        size_t size);
    zerokv::transport::Future<FetchResult> fetch(const std::string& key);
    zerokv::transport::Future<void> fetch_to(const std::string& key,
    ...
    zerokv::transport::Future<void> push(const std::string& target_node_id,
                              const std::string& key,
                              const void* data,
                              size_t size);
    zerokv::transport::Future<void> subscribe(const std::string& key);
    zerokv::transport::Future<void> unsubscribe(const std::string& key);
    zerokv::transport::Future<void> unpublish(const std::string& key);
